<h1 align="center">Hi 👋, I'm Abhimanyu Pratap</h1>
<p align="center">
...
**Answer:**  
**Steps Performed:**
1. Created a new IAM user named `abhimanyu-pw `.
2. Created a custom IAM policy to allow limited S3 access (List, Get, Put, Delete) only for the bucket: `S3Access-abhimanyu-pw-bucket`.
...
1. Created an S3 bucket named `abhimanyu-pw-bucket`.
2. Enabled versioning on the bucket through the AWS Management Console.
3. Uploaded a JSON file `abhimanyu-pw-json.json` with the original data:
...
```
4. Uploaded a second version of the same file `abhimanyu.json` with updated age:
...
1. Created and applied a lifecycle policy on the S3 bucket (`abhimanyu-pw-bucket`) to automatically transition objects to Glacier storage after 30 days and delete them after 90 day.
...
I logged into the AWS Management Console, navigated to DynamoDB, and created a new table named `AbhimanyuTable` with `id` as the Partition Key (string).
...
1. **Created an S3 bucket** named `avin-masih-log`.
...
**S3 Dataset Source**
- **Bucket Name**: `avi-pw-bucket`
- **Folder**: `csv-data/`
- **CSV File Example**: `s3://avi-pw-bucket/csv-data/pw-sample.csv`
...
```
**COPY Command Used:**
To load the CSV file from my S3 bucket `(s3://avi-pw-bucket/csv-data/sales_data.csv)` into the `sales_data` table, I used the following COPY command:

```sql
COPY sales_data
FROM 's3://avi-pw-bucket/csv-data/sales_data.csv'
...
- I uploaded my dataset (CSV files) to the S3 bucket: `s3://avi-pw-bucket/csv-data/`.
...
<footer style="text-align: center; padding: 1rem; font-size: 0.9rem; color: #666;">
  &copy; 2025 <a href="https://avi-masih.github.io/" target="_blank" style="color: inherit; text-decoration: underline;">Avinesh Masih</a>. All rights reserved.
</footer>
